...
export async function GET() {
  try {
    ...
    const output = await Promise.all(targets.map(async t => {
      ...
      ).bind(t.search_query).first<{ ts: string | null }>()

      let latest_rank: number | null = null
      if (latestRow?.ts) {
        const rr = await db.prepare(`
          SELECT rank FROM rankings WHERE search_query = ? AND collected_at = ? AND place_name LIKE ? ORDER BY rank LIMIT 1
        `).bind(t.search_query, latestRow.ts, `%${name}%`).first<{ rank: number }>()
